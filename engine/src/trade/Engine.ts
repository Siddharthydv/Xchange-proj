...
export class Engine {
        ...
        switch (message.type) {
            ...
            case CANCEL_ORDER:
                try {
                    ...
                    //cancelling the bids or asks and adding the price of unfilled order to user's account;
                    if (order.side === "buy") {
   ...
   createOrder(market: string, price: string, quantity: string, side: "buy" | "ask", userId: string) {
    ...
    updateDbOrders(order: Order, executedQty: number, fills: Fill[], market: string) {
        ...
        //to update the orders who have been filled due to the new order
        fills.forEach(fill => {
            RedisManager.getInstance().pushMessage({
                type: ORDER_UPDATE,
                data: {
    ...
    async createDbTrades(fills: Fill[], market: string, userId: string) {
        const now = Date.now();
        console.log("Timestamp:", now);
        console.log("UTC Time:", new Date(now).toISOString());
        console.log("Local Time:", new Date(now).toString());
        
        //to register the trade
        fills.forEach(fill => {
               RedisManager.getInstance().pushMessage({
                type: TRADE_ADDED,
                data: {
    ...
    async publishWsTrades(fills: Fill[], userId: string, market: string) {
        console.log("publishingwstrades=",market)
        fills.forEach(fill => {
    ...
    publisWsDepthUpdates(fills: Fill[], price: string, side: "buy" | "ask", market: string) {
        const orderbook = this.orderbooks.find(o => o.ticker() === market);
        if (!orderbook) {
        ...
        if (side === "buy") {
            ...
            const zeroquantity=fills.filter(x=>!depth?.asks.map(p=>p[0]).includes(x.price));
            zeroquantity.forEach(x=>updatedAsks.push([x.price,"0"]));
            const updatedBid = depth?.bids.find(x => x[0] === price);
            //sedning only the new bids or the modified bids,unaltered bid are not sent ;
            console.log("publish ws depth updates",updatedBid,updatedAsks)
            RedisManager.getInstance().publishMessage(`depth@${market}`, {
                stream: `depth@${market}`,
                data: {
                    a: updatedAsks,
        ...
        if (side === "ask") {
           ...
           const zeroquantity=fills.filter(x=>!depth?.bids.map(p=>p[0]).includes(x.price));
           zeroquantity.forEach(x=>updatedBids.push([x.price,"0"]));
    ...
    updateBalance(userId: string, baseAsset: string, quoteAsset: string, side: "buy" | "ask", fills: Fill[], executedQty: number) {
        if (side === "buy") {
            fills.forEach(fill => {
    ...
    checkAndLockFunds(baseAsset: string, quoteAsset: string, side: "buy" | "ask", userId: string, asset: string, price: string, quantity: string) {
        if (side === "buy") {
